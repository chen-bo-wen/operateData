...
# 图像均为cv2读取
class DataAugmentForObjectDetection():                             # 数据增强控制的地方！！！数据增强控制的地方！！！数据增强控制的地方！！！数据增强控制的地方！！！
    def __init__(self, rotation_rate=0.5, max_rotation_angle=5,
                 crop_rate=0.5, shift_rate=0.5, change_light_rate=0.9,
                 add_noise_rate=0.9, flip_rate=0.5,
    ...
    # def _changeLight(self, img):
    #     alpha = random.uniform(1.0, 1.6)  # 调整alpha的范围，使其倾向于增加亮度
    #     blank = np.zeros(img.shape, img.dtype)
    #     return cv2.addWeighted(img, alpha, blank, 1 - alpha, 0)
...
if __name__ == '__main__':
    ...
    # parser.add_argument('--source_img_path', type=str, default= r'C:\Users\Administrator\Desktop\weed\JPEGImages')
    # parser.add_argument('--source_xml_path', type=str, default= r'C:\Users\Administrator\Desktop\weed\Annotations')
    # parser.add_argument('--save_img_path', type=str, default= r'C:\Users\Administrator\Desktop\weed\upimg')
    # parser.add_argument('--save_xml_path', type=str, default= r'C:\Users\Administrator\Desktop\weed\upxml')
    # args = parser.parse_args()
    parser.add_argument('--source_img_path', type=str, default= r'/App/lpl/SELFImageSets/test_xml/jpg')
    parser.add_argument('--source_xml_path', type=str, default= r'/App/lpl/SELFImageSets/test_xml/xml')
    parser.add_argument('--save_img_path', type=str, default= r'/App/lpl/SELFImageSets/test_save_xml/jpg')
    parser.add_argument('--save_xml_path', type=str, default= r'/App/lpl/SELFImageSets/test_save_xml/xml')
    args = parser.parse_args()
    ...
    for parent, _, files in os.walk(source_img_path):
        ...
        for file in files:
            ...
            while cnt < need_aug_num:  # 继续增强
                ...
                img_name = '{}_{}{}'.format(_file_prefix, cnt + 3, _file_suffix)  # 图片保存的信息  名字顺序改这里 需修改地方
                toolhelper.save_img(img_name, save_img_path,
                                    auged_img)  # 保存增强图片

                toolhelper.save_xml('{}_{}.xml'.format(_file_prefix, cnt + 3),   # 标签保存的信息  名字顺序改这里 需修改地方
